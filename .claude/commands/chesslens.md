...
产出小红书图文素材到 `games/{GAMEDIR}/xhs/`，**全程匿名打码、停在发布前**等用户手动上传。

参数 `$ARGUMENTS`：可为 game_id、chess.com 对局 URL，或留空（=取最新一盘）。

> **单仓布局**：每盘棋一个目录 `games/{date}_{对手}_{id}/`（下称 `{GAMEDIR}`），
> 内含复盘 md + json + 棋盘图 + `xhs/` 小红书素材。`docs/` 仅是 CI 构建产物（html，gitignore）。
> 对手 = 对局中非 `aaronwang2026` 的一方。
...
- 本地：`ls -d games/*_{game_id}` 或 `ls games/*/*{game_id}*.md`
- 已有 `games/{GAMEDIR}/engine_eval.json` → 跳到第 5 步。
...
### 4. 引擎分析（Stockfish）
先按 `{date}_{对手}_{id}` 组好 `{GAMEDIR}`，分析输出直接写进去：
```bash
python3 skills/chess-analysis/scripts/analyze.py \
  --pgn-file /tmp/game_pgn_{game_id}.pgn 16 \
  --focus-user aaronwang2026 \
  --output-dir games/{GAMEDIR}/
```
产出 `games/{GAMEDIR}/` 下 `pgn.json` / `engine_eval.json` / `metadata.json`。
复盘 md 也写到 `games/{GAMEDIR}/{date}_{id}_{白}_{结果}_{黑}_{步}_{tc}.md`（保留描述性文件名，供 generate.py 解析元数据）。
...
python3 scripts/generate_blunder_images.py \
  --data-dir games/{GAMEDIR}/ \
  --output-dir games/{GAMEDIR}/xhs/ --top 3
```
...
- 写 `games/{GAMEDIR}/xhs/caption.txt`：标题钩子→背景悬念→TOP失误清单→错失速杀科普→升华→互动。
  话题标签 `#国际象棋 #棋局复盘 #chess #国象 #象棋复盘 #ChessLens`（**不带**账号标签）。
- 成品统一放 `games/{GAMEDIR}/xhs/`。

### 8. 小红书素材收尾（不发布）
**绝不自动发布。** 列出 `games/{GAMEDIR}/xhs/` 成品清单 + caption 预览，
...
`analyze.py` 把复盘 md 写进 `games/{GAMEDIR}/` → commit `games/{GAMEDIR}/`
→ push main → CI 跑 `generate.py`（读 `games/*/*.md` 构建 `docs/`）→ `deploy.yml` 自动部署。
**html 不入库，CI 在服务端构建。**
...
```
> ⚠️ 脚本会先确认 `games/` 里有本局 md；没有则**拒绝构建**（防止把首页 index 清空再 push）。

本地全绿后，**让用户确认再 push**（不自动 push）：
```bash
git add games/{GAMEDIR} && git commit -m "chore: add review for {game_id}"
